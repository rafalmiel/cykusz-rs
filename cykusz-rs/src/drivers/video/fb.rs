...
impl State {
    ...
    pub fn write_byte(&mut self, byte: u8) {
        match byte {
            b'\n' => self.new_line(),
            byte => {
                self.print_char(ScreenChar::new(byte, ColorCode::new(Color::LightGreen, Color::Black)), self.column, self.row);
                self.column += 1;
    ...
    fn clear(&mut self) {
        self.buffer.fill(0);
        self.char_cache.fill(ScreenChar::new(b' ', ColorCode::new(Color::LightGreen, Color::Black)));
    }

    fn cursor_blink_timer(&mut self) {
        self.print_cursor(self.cursor_column, self.cursor_row, !self.cursor_blink_shown);
        self.cursor_blink_shown = !self.cursor_blink_shown;

        self.cursor_timer.as_ref().cloned().unwrap().start_with_timeout(200);
    }

    fn set_cursor_visible(&mut self, vis: bool) {
        if vis && self.char_cache.is_empty() {
            self.char_cache.resize(self.txt_width * self.txt_height, ScreenChar::new(b' ', ColorCode::new(Color::LightGreen, Color::Black)));
        }
